...
class ReaderMemory(DatasetReader):
    ...
    def read_dataset(self, file_path):
        ...
        samples = json.load(open("dataset/"+ str(file_path), 'r', encoding="utf-8"))
            ...
            for _ in range(iter_num):
                for sample in all_data:
                    ...
                    if key == "pos":
                        ...
                        yield self.text_to_instance((sample, sample))  # always use the corresponding CVE to make pairs
                        for same in random.choices(dataset[sample['CWE_ID']], k = same_per_sample - 1):
                            yield self.text_to_instance((sample, same))  # CVE pair
                        ...
                        for diff in random.choices(anchor_classes, k = diff_per_sample):
                            ...
                            yield self.text_to_instance((sample, {"CWE_ID": diff, self._target: "pos"}))
    ...
    @overrides
    def text_to_instance(self, *inputs, type_="train") -> Instance:
