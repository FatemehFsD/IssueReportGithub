...
class ReaderSingle(DatasetReader):
    ...
    @overrides
    def text_to_instance(self, *inputs, type_="train") -> Instance:  # type: ignore
        # share the code between predictor and trainer, hence the label field is optional
        fields: Dict[str, Field] = {}

        fields["sample"] = TextField(inputs["description"], self._token_indexers)

        fields['label'] = LabelField(inputs[self._target], label_namespace="class_labels")
        
        meta_ins = {"Issue_Url": inputs["Issue_Url"], "label": inputs[self._target]}
