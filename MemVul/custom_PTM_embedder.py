...
class PretrainedTransformerEmbedder(TokenEmbedder) : 
    ...
    def forward(
        ...
        token_ids: torch.LongTensor = input[0]
        mask: torch.BoolTensor= input[1]
        type_ids: Optional[torch.LongTensor] =  input[2] if len(input) > 2 else  None
        segment_concat_mask: Optional[torch.BoolTensor] =  input[3] if len(input) > 3 else  None
